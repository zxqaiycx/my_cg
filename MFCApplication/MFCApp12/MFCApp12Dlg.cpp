...
{
	// TODO: 在此添加控件通知处理程序代码
	int iResult = ::MessageBox(NULL, _T("确认要关机？"), _T("关机"), MB_OKCANCEL | MB_ICONQUESTION);
	...
	if (1 == iResult)
	{
		...
		//const size_t commandSize = (command.GetLength() + 1) * 2;	// 宽字符的长度
		//char *lpszCommand = new char[commandSize];	// 分配空间
		//lpszCommand = (LPSTR)command.GetBuffer(commandSize);
		//command.ReleaseBuffer();

		((CEdit *)GetDlgItem(IDC_COUNTDOWNTIME))->GetWindowText(command);	// 从控件读取string，类型转换
